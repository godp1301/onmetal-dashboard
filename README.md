...
2. `bower install`
3. `gulp js && gulp css`
4. `cp config.sample.js config.js` edit as needed.  The options in config.sample.js are defaults and will be merged into config.js on load (config.js will overwrite if there is a conflict).  Confidential data should be kept in config.js.
5. `npm start` or `bin/www`
